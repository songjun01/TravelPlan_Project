...
---
 ...
 ## 바이브 코딩 활용 소감
 - 혼자 개발을 하며 모르는 것이 있을 때 마다 ChatGPT에 물어보는 식의 코딩은 많이 해왔지만 AI에게 모든 코드의 생성을 맡기는 식의 코딩은 처음이어서 많이 걱정되었었다. 하지만 이 프로젝트를 통해 바이브 코딩을 경험해보면서 AI에게 프롬프트만 잘 작성해주면 그리 오랜 시간을 들이지 않더라도 내가 원하는 프로젝트를 만들 수 있다는 점에 많이 놀랐고 좋은 부분이라고 생각한다.
 - 나는 AI에게 프롬프트를 더 정확하게 전달하기 위해 Gemini AI에 내가 원하는 기능을 알려주고 Gemini CLI를 위한 프롬프트를 작성해달라고 요청했다. Gemini AI를 활용하면 내가 원하는 기능을 더 명확하게 전달할 수 있고, 오류가 발생할 확률도 상대적으로 적었다.
 - [Gemini AI와의 대화 내용](https://gemini.google.com/share/93170b3d039c)

 ## 최종 결과물 평가
 - 달성한 목표: 여행 계획을 생성하고 작성하여 로컬 DB에 저장할 수 있다. 저장된 계획을 읽고, 수정하고, 삭제할 수 있으며, 즐겨찾기 기능과 정렬 기능 구현을 통해 더욱 직관적인 UX/UI를 구축하였다.
 - 미완성 기능: 계획 공유 기능
 - 향후 개선 계획: 로컬 DB가 아닌 실제 DB를 활용하고, 회원가입 기능을 추가해서 기록에 접근할 수 있게 수정한다.